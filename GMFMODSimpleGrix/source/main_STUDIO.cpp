...
{
        return (double) (degrees % 360) * (float)(3.1415926535f / 180);
}
...
FMOD_RESULT F_CALLBACK myread(void *handle, void *buffer, unsigned int sizebytes, unsigned int *bytesread, void *userdata)
{
    ...
    if (bytesread)
    {
		...
		if(userdata)
		{
			...
			{
				//(*(bytes+i)) = ((*(bytes+i)) != md->hash1[(int)(fabs(sin(degtorad(rol)))*40)]) != md->hash2[md->pat];
				//*bytes = ((*bytes) != md->hash1[(int)(fabs(sin(degtorad(rol)))*40)]) != md->hash2[md->pat];
				//*bytes = *bytes != 'A';
				//*bytes = (*bytes) != md->hash2[md->pat];
...
//    gmapi = gm::CGMAPI::Create( &gmapiresult );
    gmapiinited = false;//gmapiresult==0;
    //init_ds_list_add();
    ...
    */
    ...
    {
        FMODASSERT(FMOD_System_GetDriverCaps(mainsystem, 0, &caps, 0, &speakermode));
        ...
        //Dont care if this fails
        FMOD_System_SetHardwareChannels(mainsystem, 32);
...
//    if(FMODInternalSetResampler(samplemethod) == 0) return 0;
	...
	if(!supportwebfiles)
		FMODASSERT(FMOD_System_SetFileSystem(mainsystem,myopen, myclose, myread, myseek,NULL,NULL,2048));
...
export double FMODSoundAddEffect(double sound, double effect, double pos)
{
    if(!inited) {{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    ...
    myinfo *mi = (myinfo *) a;
 	...
 	FMODASSERT(FMOD_System_CreateDSPByType(mainsystem,(FMOD_DSP_TYPE)int(effect),&mi->moreeffects[(int)pos]));
...
export double FMODSoundSetEffects(double sound, double effects)
{
	...
	if(effects != se_none)
	{
        ...
        {
			FMODASSERT(FMOD_System_CreateDSPByType(mainsystem,FMOD_DSP_TYPE_SFXREVERB,&dsp));
			mi->reverb = dsp;
		}
...
export double FMODSoundLoop3d(double sound, double x, double y, double z, double paused)
{
	...
	if(a==NULL){{FMODASSERT(FMOD_ERR_INVALID_HANDLE);}}
	myinfo *mi = (myinfo *) a;
	if(mi->threed == FALSE && mi->streamready == false)
	{
...
export double FMODSoundSet3dMinMaxDistance(double sound, double Min, double Max)
{
	...
	myinfo *mi = (myinfo *) a;
	mi->minmaxset = TRUE;
	mi->mind = max(0,Min /**worldscale*/);
	mi->maxd = max(Max/**worldscale*/,mi->mind);
...
export double FMODSoundFree(double sound)
{
    ...
    do
    {
        ...
        UINT result = FMOD_Sound_GetOpenState((FMOD_SOUND*)(DWORD)sound,&openstate, 0, 0,0);
...
export double FMODAllSetMuted(double mute)
{
    if(!inited) {{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    FMOD_CHANNELGROUP * maingrp = NULL;
	FMODASSERT(FMOD_System_GetMasterChannelGroup(mainsystem, &maingrp));
...
export double FMODAllSetPaused(double Pause)
{
    if(!inited) {{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    FMOD_CHANNELGROUP * maingrp = NULL;
	FMODASSERT(FMOD_System_GetMasterChannelGroup(mainsystem, &maingrp));
...
export double FMODInstanceSetVolume(double instance, double volume)
{
    ...
    if(a==NULL){{FMODASSERT(FMOD_ERR_INVALID_HANDLE);}}
	myinfo *d = (myinfo*) a;
	FMODASSERT(FMOD_Channel_SetVolume((FMOD_CHANNEL *) (DWORD) instance, max(0,min(volume, 1))*d->maxvolume));
	return (double) 1;
}
...
export double FMODSoundSet3dCone(double sound, double insideconeangle, double outsideconeangle, double outsidevolume)
{
	...
	myinfo *mi = (myinfo *) a;
	mi->coneset = TRUE;
	mi->insideconeangle = insideconeangle;
	mi->insideconeangle = outsideconeangle;
	mi->outsidevolume = max(min(outsidevolume,1),0);
...
export double FMODSoundSet3dDopplerMax(double sound, double dmax)
{
    if(!inited) {{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    if(!(sound>0)){{FMODASSERT(FMOD_ERR_INVALID_HANDLE);}}
	...
	mi->maxdoppler = max(0,min(dmax,5));
...
export double FMODInstanceSetFrequency(double instance, double freq)
{
    if(!inited) {{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    if(!(instance>0)){{FMODASSERT(FMOD_ERR_INVALID_HANDLE);}}
    FMODASSERT(FMOD_Channel_SetFrequency((FMOD_CHANNEL *) (DWORD) instance, freq));
...
export double FMODGroupSetFrequency(double group, double freq)
{
    ...
    if(group == 0)
    {
		...
		FMODASSERT(FMOD_ChannelGroup_OverrideFrequency(g, freq));
	}
    else if(group == 1)
    {
		FMODASSERT(FMOD_ChannelGroup_OverrideFrequency(priority, freq));
	}
    else if(group == 2)
    {
		FMODASSERT(FMOD_ChannelGroup_OverrideFrequency(effects, freq));
	}
    else if(group == 3)
    {
		FMODASSERT(FMOD_ChannelGroup_OverrideFrequency(ambientmusic, freq));
	}
    else if(group == 4)
    {
		FMODASSERT(FMOD_ChannelGroup_OverrideFrequency(ambienteffects, freq));
	}
	return (double) 1;
}
...
export double FMODGroupSetPitch(double group,double vol)
{
    if(!inited) {{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    if(group<0 || group>4) {{FMODASSERT(FMOD_ERR_INVALID_PARAM);}}
    ...
    if(group == 0)
    {
		...
		FMODASSERT (FMOD_ChannelGroup_SetPitch(g, vol));
	}
    else if(group == 1)
    {
		FMODASSERT (FMOD_ChannelGroup_SetPitch(priority, vol));
	}
    else if(group == 2)
    {
		FMODASSERT (FMOD_ChannelGroup_SetPitch(effects, vol));
	}
    else if(group == 3)
    {
		FMODASSERT (FMOD_ChannelGroup_SetPitch(ambientmusic, vol));
	}
    else if(group == 4)
    {
		FMODASSERT (FMOD_ChannelGroup_SetPitch(ambienteffects, vol));
	}
	return (double) 1;
...
export double FMODInstanceSetPan(double instance, double p)
{
	...
	if(p<-1) {{FMODASSERT(FMOD_ERR_INVALID_PARAM);}};
	if(p>1) {{FMODASSERT(FMOD_ERR_INVALID_PARAM);}};
 	FMODASSERT(FMOD_Channel_SetPan((FMOD_CHANNEL *) (DWORD) instance, p));
	return (double) 1;
}
...
export double FMODInstanceSet3dCone(double instance, double insideconeangle, double outsideconeangle, double outsidevolume)
{
	...
	FMODASSERT(FMOD_Channel_Set3DConeSettings((FMOD_CHANNEL *)(DWORD)instance, insideconeangle,outsideconeangle,outsidevolume));
    return (double) 1;
}
...
export double FMODBlockersInit(double NumBlockers, double xs, double ys, double zs)
{
    ...
    FMODASSERT(FMOD_System_SetGeometrySettings(mainsystem, (1.5*(max(xs,max(ys,max(zs,100))))))/2);
...
export double FMODBlockerSet3dOrientation(double blocker, double fx, double fy, double fz, double ux, double uy, double uz)
{
    ...
    if(geometry == NULL){{FMODASSERT(FMOD_ERR_INITIALIZATION);}}
    if(!(blocker >0)){{FMODASSERT(FMOD_ERR_INVALID_HANDLE);}}
    FMOD_VECTOR fv = {fx,fy,fz};
    FMOD_VECTOR uv = {ux,uy,uz};
...
export double FMODBlockerSetStrength(double blocker,double strength)
{
    ...
    if(polies>0)
    {
		...
		{
	    	FMODASSERT(FMOD_Geometry_SetPolygonAttributes((FMOD_GEOMETRY*)(DWORD)blocker, i, strength, strength, 1));
		}
	    return (double) 1;
	}
	return (double) 0;
...
export double FMODGroupSetPan(double group, double pan)
{
    ...
    if(group == 0)
    {
		...
		FMODASSERT(FMOD_ChannelGroup_OverridePan(g, pan));
	}
    else if(group == 1)
    {
		FMODASSERT(FMOD_ChannelGroup_OverridePan(priority, pan));
	}
    else if(group == 2)
    {
		FMODASSERT(FMOD_ChannelGroup_OverridePan(effects, pan));
	}
    else if(group == 3)
    {
		FMODASSERT(FMOD_ChannelGroup_OverridePan(ambientmusic, pan));
	}
    else if(group == 4)
    {
		FMODASSERT(FMOD_ChannelGroup_OverridePan(ambienteffects, pan));
	}
	return (double) 1;
}
...
export double FMODEncryptFile(LPCSTR sourcename, LPCSTR destname, LPCSTR password)
{
char hash1[1024];
memset(hash1,0,1024);
CSHA1 sha1;
...
//return;
	...
	{
        c = fgetc(sfile );
        //fputc((c != hash2[at]) != hash1[(int)(fabs(sin(degtorad(rol)))*lnr)],dfile);
        fputc((c != hash2[at]) != hash1[at],dfile);
        //fputc(c != 'A',dfile);
        //fputc(c,dfile);
        //fputc(c != hash2[at],dfile);
        at++;
        if(at >= ln) at = 0;
        c = 0;
        rol++;
...
export double FMODInstanceAddEffect(double instance, double effect)
{
    ...
    if((effect>FMOD_DSP_TYPE_LOWPASS_SIMPLE)) {{FMODASSERT(FMOD_ERR_INVALID_PARAM);}}
    FMOD_DSP *eff = NULL;
 	FMODASSERT(FMOD_System_CreateDSPByType(mainsystem,(FMOD_DSP_TYPE)int(effect),&eff));
...
export double FMODInstanceSetSpeakerMix(double instance,
  double  frontleft,
  double  frontright,
  double  center,
  double  lfe,
  ...
  double  sideright)
{
	...
	FMODASSERT(FMOD_Channel_SetSpeakerMix((FMOD_CHANNEL *) (DWORD) instance,
        max(0,min(5,frontleft)),
        max(0,min(5,frontright)),
        max(0,min(5,center)),
        max(0,min(5,lfe)),
        max(0,min(5,backleft)),
        max(0,min(5,backright)),
        max(0,min(5,sideleft)),
        max(0,min(5,sideright))
    ));
	return (double) 1;
...
export double FMODInstanceSet3DSpread(double instance, double spreadangle)
{
	...
	float a = max(0,min(360,spreadangle));
...
export double FMODInstanceGetNextTag(double instance)
{
    ...
    else if (tag.datatype == FMOD_TAGDATATYPE_FLOAT)
    {
        ...
        wsprintf(tagdata,"%f",*f);
        return (double)1;
    }
    else
    {
        int *v = (int*)tag.data;
        wsprintf(tagdata,"%d",*v);
        return (double)1;
    }
    return (double)2;
}
...
export double FMODInstanceAsyncOK(double instance)
{
    ...
    FMOD_BOOL diskbusy = false;
    FMODASSERT(FMOD_Sound_GetOpenState(sound,&openstate,NULL,&starving,&diskbusy));
    if(openstate == FMOD_OPENSTATE_ERROR) FMODASSERT(FMOD_ERR_HTTP_TIMEOUT);

    return (double) (!starving && !diskbusy);
...
export double FMODSoundAsyncReady(double sound)
{
    ...
    if(!(sound>0)){{FMODASSERT(FMOD_ERR_INVALID_HANDLE);}}
    FMOD_OPENSTATE openstate = FMOD_OPENSTATE_READY;
    FMODASSERT(FMOD_Sound_GetOpenState((FMOD_SOUND *)(DWORD)sound,&openstate,NULL,NULL,NULL));
    if(openstate == FMOD_OPENSTATE_ERROR) FMODASSERT(FMOD_ERR_HTTP_TIMEOUT);
...
export const char * FMODGetErrorString(double err_code)
{
    return FMOD_ErrorString((FMOD_RESULT)int(err_code));
}
/*
...
FMOD_RESULT F_CALLBACK ReFillBuffer(FMOD_SOUND*, void *data, unsigned int datalen)
{
    if (!semaphore) {
        memcpy(data,(char*)gm_data+gm_len,gm_len);
        return FMOD_OK;
    }

    semaphore=0;
    
    memcpy(data,gm_data,gm_len);

    return FMOD_OK;
}

export double FMODBufferSoundSemaphore() {
    semaphore=1;
    return 0;
}
